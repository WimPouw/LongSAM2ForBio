...
# Fix threading issues for different environments
os.environ["OMP_NUM_THREADS"] = "1"
os.environ["MKL_NUM_THREADS"] = "1" 
os.environ["NUMEXPR_NUM_THREADS"] = "1"
os.environ["VECLIB_MAXIMUM_THREADS"] = "1"

# Fix OpenCV threading issues
os.environ["OPENCV_VIDEOIO_PRIORITY_MSMF"] = "0"
os.environ["OPENCV_VIDEOIO_MSMF_ENABLE_HW_TRANSFORMS"] = "0"

# PyTorch threading fixes
os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"
os.environ["CUDA_LAUNCH_BLOCKING"] = "1"

# Prevent GUI threading issues
import warnings
warnings.filterwarnings("ignore", category=UserWarning)

# Import OpenCV with threading fix
import cv2
# Force single-threaded OpenCV
cv2.setNumThreads(1)

# Fix for different Python environments
try:
    import matplotlib
...
def fix_environment_compatibility():
    """Fix common environment compatibility issues"""
    try:
        # Set single-threaded execution for stability
        import torch
        torch.set_num_threads(1)
        
        # Disable OpenMP for stability
...
# Alternative OpenCV window creation (more stable)
def create_opencv_window_safe(window_name, flags=cv2.WINDOW_NORMAL):
    """Create OpenCV window with error handling"""
    try:
        cv2.namedWindow(window_name, flags)
        return True
    except Exception as e:
        print(f"⚠️ OpenCV window creation failed: {e}")
        print("This may be due to display/GUI issues on this system")
        return False

def safe_opencv_operations():
    """Make OpenCV operations safer for different environments"""
    try:
        ...
        print("⚠️ OpenCV GUI not available - using fallback mode")
        return False

# Apply fixes immediately
fix_environment_compatibility()
import os
import sys
...
# Update the analyze_frame_overlaps method to use "touch" terminology
    def analyze_frame_overlaps(self, frame_results, object_names):
        """Frame analysis for TOUCH EVENTS - when target touches/contacts objects"""
        frame_analysis = {
            'target_overlaps': {},
            'object_relationships': {},
            'touch_events': []  # Changed from 'looking_at_events'
        }
        
        print(f"\n🔍 ANALYZING FRAME FOR TOUCH EVENTS:")
        print(f"  Total objects in frame: {len(frame_results)}")
        print(f"  Registered targets: {list(self.target_objects.keys())}")
        print(f"  Object names: {object_names}")
        
        try:
            for target_id in self.target_objects:
                if target_id not in frame_results:
                    print(f"  ⚠️ Target {target_id} not found in frame results")
                    continue
                    
                target_mask = frame_results[target_id]
                target_name = self.target_objects[target_id]
                touching_objects = []  # Changed from 'looking_at_objects'
                
                print(f"\n  🎯 Checking target '{target_name}' (ID: {target_id}) for touch events")
                print(f"    Target mask shape: {target_mask.shape}")
                
                # Ensure target mask is 2D
                if len(target_mask.shape) > 2:
                    target_mask = target_mask.squeeze()
                    print(f"    Squeezed to: {target_mask.shape}")
                
                target_pixels = np.sum(target_mask > 0)
                print(f"    Target has {target_pixels} pixels")
                
                # Check for TOUCH events with all other objects
                for obj_id, mask in frame_results.items():
                    if obj_id == target_id:
                        continue
                        
                    obj_name = object_names.get(obj_id, f"Object_{obj_id}")
                    print(f"    Testing for touch with '{obj_name}' (ID: {obj_id})")
                    
                    # Ensure object mask is 2D
                    if len(mask.shape) > 2:
                        mask = mask.squeeze()
                    
                    obj_pixels = np.sum(mask > 0)
                    print(f"      Object has {obj_pixels} pixels")
                    
                    try:
                        # Use the detector for touch detection
                        overlap_info = self.detector.calculate_detailed_overlap(target_mask, mask)
                        
                        if overlap_info:
                            print(f"      ✅ TOUCH DETECTED WITH {obj_name}!")
                            
                            touching_objects.append({
                                'object_id': obj_id,
                                'object_name': obj_name,
                                'event_type': 'touch',  # Changed from 'looking_at'
                                'relationship_desc': f"TOUCHING {obj_name}"  # Changed from "LOOKING AT"
                            })
                            
                            frame_analysis['touch_events'].append({  # Changed from 'looking_at_events'
                                'target_id': target_id,
                            ...
                            print(f"      ❌ No touch with {obj_name}")
                ...
                if touching_objects:
                    frame_analysis['target_overlaps'][target_id] = touching_objects
                    print(f"  ✅ Target {target_name} found {len(touching_objects)} touch events")
                else:
                    print(f"  ❌ Target {target_name} found NO touch events")
            
            print(f"\n📊 Frame analysis complete:")
            print(f"  Total touch_events: {len(frame_analysis['touch_events'])}")
    ...
    def _update_overlap_event(self, target_id, frame_idx, overlapping_names):
        ...
        if events and events[-1].get('end_frame') is None:
            ...
            if current_overlap_set == last_overlap_set and current_overlap_set:
                ...
                print(f"      ✅ TOUCH EVENT ENDED: {target_name} frames {last_event['start_frame']}-{last_event['end_frame']} ({last_event['duration_frames']} frames) | touching {objects_str}")
                
                # Start new touch event if we have new contacts
                if current_overlap_set:
                    ...
                    print(f"      🤝 NEW TOUCH EVENT: {target_name} frame {frame_idx} | touching {objects_str}")
        else:
            # No ongoing event - start new touch event if we have contact
            if current_overlap_set:
                new_event = {
                    'start_frame': frame_idx,
                    'end_frame': None,
                ...
                events.append(new_event)
                objects_str = ', '.join(current_overlap_set)
                print(f"      🤝 FIRST TOUCH EVENT: {target_name} frame {frame_idx} | touching {objects_str}")
...
class UltraOptimizedProcessor:
    ...
    def __init__(self, predictor, video_dir, overlap_threshold=0.1, reference_frame=0, 
        ...
        self.csv_data = []
        self.detailed_csv_data = []
    def calculate_object_centroid(self, mask):
    ...
    def collect_frame_data_for_csv(self, frame_idx, frame_results, object_names, frame_analysis):
        """Collect comprehensive frame data for CSV export with TOUCH information"""
        try:
            # Basic frame info
            frame_data = {
                'frame_index': frame_idx,
                'timestamp_seconds': frame_idx / 30.0,  # Assuming 30 FPS, will be corrected later
                'total_objects': len(frame_results),
                'has_touch_events': bool(frame_analysis.get('target_overlaps'))
            }
            
            # Object position and area data with TOUCH information
            for obj_id, mask in frame_results.items():
                obj_name = object_names.get(obj_id, f"Object_{obj_id}")
                is_target = obj_id in self.overlap_tracker.target_objects
                
                # Calculate centroid and area
                cx, cy, area = self.calculate_object_centroid(mask)
                
                # Add object-specific data
                frame_data[f'{obj_name}_centroid_x'] = cx if cx is not None else np.nan
                frame_data[f'{obj_name}_centroid_y'] = cy if cy is not None else np.nan
                frame_data[f'{obj_name}_area'] = area
                frame_data[f'{obj_name}_is_target'] = is_target
                
                # Add target-specific TOUCH data
                if is_target:
                    target_touches = frame_analysis.get('target_overlaps', {}).get(obj_id, [])
                    frame_data[f'{obj_name}_touching_count'] = len(target_touches)
                    frame_data[f'{obj_name}_touching_objects'] = ', '.join([obj['object_name'] for obj in target_touches])
                    frame_data[f'{obj_name}_is_touching'] = len(target_touches) > 0
                else:
                    frame_data[f'{obj_name}_touching_count'] = 0
                    frame_data[f'{obj_name}_touching_objects'] = ''
                    frame_data[f'{obj_name}_is_touching'] = False
                
                # Add TOUCHED information (whether this object is being touched by any target)
                is_touched = False
                touched_by_targets = []
                
                for target_id, touches in frame_analysis.get('target_overlaps', {}).items():
                    for touch in touches:
                        if touch['object_id'] == obj_id:
                            is_touched = True
                            target_name = self.overlap_tracker.target_objects.get(target_id, f"Target_{target_id}")
                            touched_by_targets.append(target_name)
                
                frame_data[f'{obj_name}_is_touched'] = is_touched
                frame_data[f'{obj_name}_touched_by'] = ', '.join(touched_by_targets)
            
            # Event summary for this frame
            total_events = len(frame_analysis.get('touch_events', []))
            frame_data['total_events_this_frame'] = total_events
            
            # Add to CSV data
            self.csv_data.append(frame_data)
            
            # Detailed event data (separate rows for each event)
            for event in frame_analysis.get('touch_events', []):
                detailed_data = {
                    'frame_index': frame_idx,
                ...
                # Add target position
                if event['target_id'] in frame_results:
                    target_mask = frame_results[event['target_id']]
                    tx, ty, ta = self.calculate_object_centroid(target_mask)
                    detailed_data['target_centroid_x'] = tx if tx is not None else np.nan
                    detailed_data['target_centroid_y'] = ty if ty is not None else np.nan
                    detailed_data['target_area'] = ta
                
                # Add object position
                if event['object_id'] in frame_results:
                    obj_mask = frame_results[event['object_id']]
                    ox, oy, oa = self.calculate_object_centroid(obj_mask)
                    detailed_data['object_centroid_x'] = ox if ox is not None else np.nan
                    detailed_data['object_centroid_y'] = oy if oy is not None else np.nan
                    detailed_data['object_area'] = oa
                
                # Calculate distance if both positions are available
                if (detailed_data.get('target_centroid_x') is not None and 
                    detailed_data.get('object_centroid_x') is not None):
                    distance = np.sqrt((detailed_data['target_centroid_x'] - detailed_data['object_centroid_x'])**2 + 
                                    (detailed_data['target_centroid_y'] - detailed_data['object_centroid_y'])**2)
                    detailed_data['distance_pixels'] = distance
                else:
                    detailed_data['distance_pixels'] = np.nan
                
                self.detailed_csv_data.append(detailed_data)
            
        except Exception as e:
            print(f"⚠️ Error collecting CSV data for frame {frame_idx}: {e}")

    def save_csv_data(self, output_dir, fps=30):
        """Save comprehensive CSV data with timing correction"""
        try:
            print(f"💾 Saving CSV data with {len(self.csv_data)} frames...")
            
            if not self.csv_data:
                print("⚠️ No CSV data to save")
                return
            
            # Correct timestamps with actual FPS
            for data in self.csv_data:
                data['timestamp_seconds'] = data['frame_index'] / fps
            
            for data in self.detailed_csv_data:
                data['timestamp_seconds'] = data['frame_index'] / fps
            
            # Save main CSV file (frame-by-frame summary)
            csv_path = os.path.join(output_dir, "frame_by_frame_analysis.csv")
            df = pd.DataFrame(self.csv_data)
            df.to_csv(csv_path, index=False)
            print(f"✅ Frame-by-frame analysis saved: {csv_path}")
            
            # Save detailed events CSV
            if self.detailed_csv_data:
                detailed_csv_path = os.path.join(output_dir, "detailed_touch_events.csv")
                detailed_df = pd.DataFrame(self.detailed_csv_data)
                detailed_df.to_csv(detailed_csv_path, index=False)
                print(f"✅ Detailed touch events saved: {detailed_csv_path}")
            
            # Save event summary CSV
            if self.overlap_tracker.has_targets():
                self.save_event_summary_csv(output_dir, fps)
                
            # Save object movement analysis
            self.save_movement_analysis_csv(output_dir, fps)
            
            print(f"📊 CSV Export Summary:")
            print(f"  • frame_by_frame_analysis.csv: {len(self.csv_data)} frames")
            print(f"  • detailed_touch_events.csv: {len(self.detailed_csv_data)} events")
            print(f"  • touch_event_summary.csv: Event duration analysis")
            print(f"  • movement_analysis.csv: Object movement tracking")
    ...
    def save_event_summary_csv(self, output_dir, fps):
        """Save event summary CSV with duration analysis"""
        try:
            ...
            for target_name, data in summary.items():
                for i, event in enumerate(data['events']):
                    ...
                    event_data = {
                        ...
                        'touching_objects': ', '.join(event['overlapping_objects']),
                        'num_touching_objects': len(event['overlapping_objects'])
                    }
                    summary_data.append(event_data)
            
            if summary_data:
                summary_csv_path = os.path.join(output_dir, "touch_event_summary.csv")
                summary_df = pd.DataFrame(summary_data)
                summary_df.to_csv(summary_csv_path, index=False)
                print(f"✅ Touch event summary saved: {summary_csv_path}")
            
        except Exception as e:
            print(f"❌ Error saving event summary CSV: {e}")

    def save_movement_analysis_csv(self, output_dir, fps):
        """Save movement analysis CSV with velocity and acceleration"""
        try:
            if not self.csv_data:
                return
                
            # Collect movement data for each object
            movement_data = []
            df = pd.DataFrame(self.csv_data)
            ...
            for col in df.columns:
                if col.endswith('_centroid_x'):
                    obj_name = col.replace('_centroid_x', '')
                    object_names.add(obj_name)
            
            for obj_name in object_names:
                x_col = f'{obj_name}_centroid_x'
                y_col = f'{obj_name}_centroid_y'
                area_col = f'{obj_name}_area'
                
                if x_col in df.columns and y_col in df.columns:
                    # Get positions
                    x_data = df[x_col].values
                    y_data = df[y_col].values
                    area_data = df[area_col].values if area_col in df.columns else np.full(len(x_data), np.nan)
                    ...
                    for i in range(len(x_data)):
                        ...
                        curr_x = x_data[i]
                        curr_y = y_data[i]
                        curr_area = area_data[i]
                        
                        # Calculate velocity (if previous frame exists)
                        velocity = np.nan
                        if i > 0 and not np.isnan(x_data[i-1]) and not np.isnan(curr_x):
                            prev_x = x_data[i-1]
                            prev_y = y_data[i-1]
                        ...
                        acceleration = np.nan
                        if i > 1 and not np.isnan(x_data[i-2]) and not np.isnan(x_data[i-1]) and not np.isnan(curr_x):
                            prev_prev_x = x_data[i-2]
                            prev_prev_y = y_data[i-2]
                            prev_x = x_data[i-1]
                            prev_y = y_data[i-1]
                            
                            prev_distance = np.sqrt((prev_x - prev_prev_x)**2 + (prev_y - prev_prev_y)**2)
                            curr_distance = np.sqrt((curr_x - prev_x)**2 + (curr_y - prev_y)**2)
                            
                            prev_velocity = prev_distance * fps
                            curr_velocity = curr_distance * fps
                            acceleration = (curr_velocity - prev_velocity) * fps
                        ...
                        movement_data.append({
                            ...
                            'centroid_y': curr_y,
                            'area': curr_area,
                            'velocity_pixels_per_second': velocity,
                            'acceleration_pixels_per_second_squared': acceleration,
                            'is_visible': not np.isnan(curr_x)
                        })
            
            if movement_data:
                movement_csv_path = os.path.join(output_dir, "movement_analysis.csv")
                movement_df = pd.DataFrame(movement_data)
                movement_df.to_csv(movement_csv_path, index=False)
                print(f"✅ Movement analysis saved: {movement_csv_path}")
                
        except Exception as e:
            print(f"❌ Error saving movement analysis CSV: {e}")
            ...
            for out_frame_idx, out_obj_ids, out_mask_logits in self.predictor.propagate_in_video(inference_state):
                try:
                    ...
                    if targets_found:
                        ...
                        # Collect CSV data for this frame
                        self.collect_frame_data_for_csv(out_frame_idx, frame_results, object_names, frame_analysis)
                        ...
                        # Still collect basic CSV data even without targets
                        empty_analysis = {'target_overlaps': {}, 'touch_events': []}
                        self.collect_frame_data_for_csv(out_frame_idx, frame_results, object_names, empty_analysis)
    ...
    def save_results_video_with_enhanced_annotations(self, results, output_path, fps=30, show_original=True, alpha=0.5):
        """Save results video with TOUCH EVENT visual feedback"""
        if not results:
        ...
        print("💾 Saving video with TOUCH EVENT visual feedback...")
        touch_frame_count = 0
        ...
        for frame_idx in tqdm(range(len(self.frame_names)), desc="Saving frames"):
            ...
            # Check if this frame has touch events
            has_touch_events = (frame_analysis and 
                            frame_analysis.get('target_overlaps') and 
                            any(frame_analysis['target_overlaps'].values()))
            if has_touch_events:
                touch_frame_count += 1
            
            # Collect all touch information for this frame
            touch_info = {}  # obj_id -> {is_target: bool, touching: [objects], touched_by: [targets]}
            ...
            if frame_analysis:
                ...
                # Process target touch events
                for target_id, touching_objects in frame_analysis.get('target_overlaps', {}).items():
                    if target_id in touch_info:
                        touch_info[target_id]['is_target'] = True
                        touch_info[target_id]['touching'] = [obj['object_name'] for obj in touching_objects]
                    
                    # Mark objects being touched
                    for obj_info in touching_objects:
                        obj_id = obj_info['object_id']
                        if obj_id in touch_info:
                            target_name = self.overlap_tracker.target_objects.get(target_id, f"Target_{target_id}")
                            touch_info[obj_id]['touched_by'].append(target_name)
                            touch_info[obj_id]['is_being_touched'] = True
            
            # Apply masks with TOUCH EVENT visual feedback
            if frame_idx in results:
                for obj_id, mask in results[frame_idx].items():
                    ...
                    if mask.shape == (height, width):
                        obj_info = touch_info.get(obj_id, {})
                        is_target = obj_info.get('is_target', False)
                        is_being_touched = obj_info.get('is_being_touched', False)
                        touching = obj_info.get('touching', [])
                        touched_by = obj_info.get('touched_by', [])
                        
                        # Choose colors based on touch status
                        base_color = np.array(cmap(obj_id % 10)[:3]) * 255
                        
                        if is_target and touching:
                            # Target that's touching something - bright highlight
                            color = np.minimum(base_color + [100, 100, 0], 255)  # Yellow tint for active targets
                            border_color = (0, 255, 255)  # Cyan border for touching targets
                            border_thickness = 8
                        elif is_being_touched:
                            # Object being touched - red highlight
                            color = np.minimum(base_color + [120, 0, 0], 255)  # Red tint
                            border_color = (0, 0, 255)  # RED BORDER for touched objects
                            border_thickness = 8
                        else:
                            # Normal object
                            color = base_color
                        ...
                        if moments['m00'] != 0:
                            ...
                            if is_target and touching:
                                if len(touching) == 1:
                                    label = f"🎯{obj_name} → TOUCHING {touching[0]}"
                                else:
                                    label = f"🎯{obj_name} → TOUCHING {len(touching)} OBJECTS"
                            elif is_being_touched:
                                if len(touched_by) == 1:
                                    label = f"{obj_name} ← TOUCHED BY {touched_by[0]}"
                                else:
                                    label = f"{obj_name} ← TOUCHED BY {len(touched_by)} TARGETS"
                            else:
                                if is_target:
                                    label = f"🎯{obj_name}"
                                else:
                            ...
                            # Enhanced text rendering based on touch status
                            ...
                            # Enhanced background and text colors for touch events
                            if is_target and touching:
                                bg_color = (0, 100, 200)  # Orange background for touching targets
                                text_color = (0, 255, 255)  # Bright cyan text
                                padding = 10
                            elif is_being_touched:
                                bg_color = (0, 0, 200)  # Red background for touched objects
            ...
            # Add prominent frame status display for TOUCH EVENTS
            status_messages = []
            
            if has_touch_events:
                # Collect all touch events for display
                target_events = []
                if frame_analysis:
                    for target_id, touching_objects in frame_analysis.get('target_overlaps', {}).items():
                        target_name = self.overlap_tracker.target_objects.get(target_id, f"Target_{target_id}")
                        object_names_list = [obj['object_name'] for obj in touching_objects]
                        
                        if len(object_names_list) == 1:
                            target_events.append(f"{target_name} → {object_names_list[0]}")
                        else:
                            target_events.append(f"{target_name} → {len(object_names_list)} objects")
                
                status_messages = [f"🤝 TOUCH EVENT DETECTED: {'; '.join(target_events)}"]
            
            # Draw status messages
            info_y = 30
            for i, message in enumerate(status_messages):
                if has_touch_events:
                    # Prominent background for touch events
                    text_size = cv2.getTextSize(message, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[0]
                    cv2.rectangle(overlay, (5, info_y - 25), (text_size[0] + 15, info_y + 10), (0, 100, 0), -1)  # Green background
        ...
        out.release()
        print(f"✅ Video saved with TOUCH EVENT visual feedback: {output_path}")
        print(f"🤝 Touch events detected in {touch_frame_count} frames")
        print(f"📊 Visual enhancements:")
        print(f"  • RED BORDERS on objects being touched")
        print(f"  • CYAN BORDERS on targets that are touching")
        print(f"  • Clear touch status text for each object")
        print(f"  • Prominent touch event announcements")
        
    def create_elan_file(self, video_path, output_path, fps, frame_offset=0):
        """Create ELAN file for TOUCH EVENTS with corrected timing"""
        if not self.overlap_tracker.has_targets():
            print("No targets found - skipping ELAN export")
            return
        
        print(f"Creating ELAN file for TOUCH EVENTS: {output_path}")
        ...
        # Create ELAN XML for TOUCH EVENTS
        header = f'''<?xml version="1.0" encoding="UTF-8"?>
    <ANNOTATION_DOCUMENT AUTHOR="SAM2_Touch_Events" DATE="{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}" FORMAT="3.0" VERSION="3.0"
    ...
    '''
        ...
        # Create tiers for TOUCH EVENTS
        tier_content = ""
        annotation_id = 1
        
        for target_name, target_data in summary.items():
            tier_id = target_name.upper().replace(' ', '_').replace('-', '_')
            tier_content += f'    <TIER DEFAULT_LOCALE="en" LINGUISTIC_TYPE_REF="default" TIER_ID="{tier_id}_TOUCH">\n'
            ...
            for event in target_data['events']:
                ...
                # Create TOUCH EVENT annotation
                overlapping_objects_str = ", ".join(event['overlapping_objects'])
                duration_seconds = end_time - start_time
                
                if len(event['overlapping_objects']) == 1:
                    annotation_value = f"Touching: {overlapping_objects_str}"
                else:
                    annotation_value = f"Touching {len(event['overlapping_objects'])} objects: {overlapping_objects_str}"
        ...
        print(f"✅ ELAN file created for TOUCH EVENTS: {output_path}")
        
        # Print timing verification for TOUCH EVENTS
        print(f"\n📄 TOUCH EVENT Timing Verification:")
        for target_name, target_data in summary.items():
            if target_data['events']:
                ...
                for i, event in enumerate(target_data['events'][:3]):
                    ...
                    print(f"    Touch Event {i+1}: {start_time:.3f}s - {end_time:.3f}s | touching {objects}")
                    print(f"                       (frames {start_frame_corrected} - {end_frame_corrected})")
                
                if len(target_data['events']) > 3:
                    print(f"    ... and {len(target_data['events'])-3} more touch events")
    ...
    def process_video(self):
        ...
        try:
            ...
            if results:
                ...
                # Save CSV files
                csv_created = False
                self.status_var.set("📊 Exporting comprehensive CSV files...")
                self.root.update()
                processor.save_csv_data(frames_dir, fps)
                csv_created = True
